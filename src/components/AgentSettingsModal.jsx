...
const AgentSettingsModal = ({ agent, isOpen, onClose, onUpdate }) => {
    ...
    useEffect(() => {
        if (isOpen && agent) {
            ...
            getToken()
                .then((token) => ({ token }))
                .then(({ token }) => {
                    return apiAuthFetch(`/api/agents?action=models&t=${timestamp}`, {
                        headers: {
            ...
            getToken()
                .then((token) => ({ token }))
                .then(({ token }) => {
                    return apiAuthFetch(`/api/agents?id=${agent.id}&t=${timestamp}`, {
                        headers: {
                            ...(token ? { Authorization: `Bearer ${token}` } : {})
                        }
                    }).then(res => res.json());
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await apiAuthFetch(`/api/agents?id=${agent.id}`, {
                method: 'PATCH',
                headers: {
